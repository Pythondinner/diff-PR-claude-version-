...
**接入顺序会直接决定监控生不生效，一定要先看这条：**

> 必须**先跑 `python check.py` 接好监控，再打开 Claude Code 开始跟它聊**。反过来——已经开着 Claude Code 聊了一半，才想起来接监控——`check.py` 会正常把配置写进目标项目的 `.claude/settings.json`，但**那个已经在运行的 Claude Code 进程不会自动发现这份新配置**，实测过 `/hooks` 在这种情况下也没能让它生效，唯一确认有效的办法是把那个 Claude Code 会话完全关掉、重新打开。这大概率是 Claude Code 故意的安全设计（Hook 本质是无条件执行配置里写的命令，运行中途悄悄热加载新配置是个真实的风险面），不是这个项目能绕开的限制。先接后聊，就完全不会遇到这个问题。
...
- **中文项目名会把分区目录名压成一串短横线，两个不同项目可能撞车**：项目路径分区用的规则是"非字母数字字符全部换成短横线"，中文字符全部命中这条规则，导致"自动剧本生成机"这类项目名被清洗成完全看不出内容的 `C--Users-xxx-Desktop--------`。排查一次"接了监控但看不到反应"时意外发现的，不是理论推演——真实用户的项目名撞上了这个规则。修复：可读前缀继续保留，额外拼上一段基于完整原始路径算出的 8 位 hash 后缀保证绝对不撞车，同时对已有的几个真实分区（包括 313 条历史记录的项目自己）做了一次性迁移，逐个核对了迁移前后的记录数一致。
...
- **接入必须在打开 Claude Code 之前完成，不支持热接入**（见上方"运行方式"里的强调）——已经实测确认，`/hooks` 没法让已经在运行的会话发现新写入的 Hook 配置，唯一确认有效的办法是完全重启那个 Claude Code 会话。这个限制来自 Claude Code 自己的 Hook 加载机制，不是这个项目能绕开的
